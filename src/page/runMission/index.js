...
export default function index({ navigation, route }) {
  ...
  const datamis = route.params.datamis;
  ...
  async function allbanner() {
    ...
    const getbanner1 = await getBanNer({ token, page: "pagerun" });
    setbanner1(getbanner1.data[0].img_list);
    const getbanner2 = await getBanNer({ token, page: "pageevent" });
  ...
  useEffect(() => {
    try {
      setLastdistance(datamis[0].last_distance);
    } catch (error) {}
  }, []);
  ...
  async function callApi() {
    if (data.length == 0) {
      const resposne = await apiservice({
        ...
        body: {
          event_id: null,
          mission_id: dataEV.id,
          img_event: dataEV.img_List[0],
      ...
      // console.log("90", resposne.data.data.id);
      ...
      // มันควรเป็นการ get ไอดีแล้วเสร็จลง setmissionid() เพื่อเอาไปใช้
    }
  }
            ...
            >
              ...
              <TouchableOpacity
                onPress={async () => {
                  ...
                  if (response.status == 200) {
                    setput(false);
                    navigation.navigate("MissionFrom", { dataEV });
                  }
                }}
                style={{
                  width: 100,
            ...
            <View style={styles.viewimg}>
              ...
              <View style={styles.viewtext}>
                ...
                {datamis[0] != undefined ? (
                  ...
                  <View style={styles.viewsmall1}>
                    {dataEV.last_distance != undefined &&
                    dataEV.last_distance != "NaN" ? (
                      <Text style={styles.text1}>
                        {(
                ...
                {datamis[0] != undefined ? (
                  <View style={styles.viewsmall}>
                    <Text style={styles.text1}>
                      {(
                        ((datamis[0].last_distance / 1000).toFixed(2) * 100) /
                        (datamis[0].total_distance / 1000)
                      ).toFixed(2)}{" "}
                      %
                    </Text>
                  </View>
